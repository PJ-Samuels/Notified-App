    ...
    request.post(authOptions, function(error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        //res.send(`http://localhost:3000/user_dashboard?accesstoken=${access_token}&refreshtoken=${refresh_token}&user_id=${user_id}&expiration_time=${expiration_time}`);
         res.redirect(`https://notified-webapp-0f26d6f34016.herokuapp.com/apiuser_dashboard?accesstoken=${access_token}&refreshtoken=${refresh_token}&user_id=${user_id}&expiration_time=${expiration_time}`);
